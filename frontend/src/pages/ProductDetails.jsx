...
export default function ProductDetail() {
    ...
    const recommendation = products.filter(p => String(p.id) !== id).slice(0, 4);
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                        ...
                        <AnimatePresence>
                            {zoomOpen && (
                                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center">
                                    ...
                                    <motion.div initial={{ y: 20 }} animate={{ y: 0 }} exit={{ y: 20 }} className="relative w-[100%] h-[100%]  rounded-2xl p-6 z-10">
                                        <button onClick={() => setZoomOpen(false)} className="absolute right-4 top-4 bg-white p-2 rounded-full shadow cursor-pointer"><X /></button>
                                        <div className="flex flex-col md:flex-row gap-4 justify-center items-center h-[90vh]">
                                            <img src={images[active]} className="w-[100%] h-[100%] justify-center flex object-contain" alt="zoom" />
                                        </div>
                                    </motion.div>
                                </motion.div>
                            )}
                            ...
                            <div className="mt-8">
                                ...
                                <div className="mt-4 grid grid-cols-2 sm:grid-cols-4 gap-4">
                                    {recommendation.map((r, i) => (
                                        <Link
                                            key={r.id || i}
                                            to={`/products/${category}/${r.id}`}
                                            className="border  border-gray-300 rounded-lg p-3 hover:shadow-md transition-shadow bg-white"
                                        >
                                            <img
                                                src={r.img}
                                                ...
                                                loading="lazy"
                                            />
                                            <h4 className="mt-2 text-sm font-medium line-clamp-2">{r.title}</h4>
                    ...
                    <aside className="lg:col-span-5">
                        <div className="sticky top-24 bg-white border border-gray-300 rounded-2xl p-6 shadow">
                            <div className="flex items-start justify-between gap-3">
                                ...
                                <button onClick={toggleWishlist} aria-pressed={inWishlist} className="p-2 rounded-md">
                                    <Heart className={`${inWishlist ? 'text-red-500 fill-red-500' : 'text-gray-400'}`} />
                                </button>
                            </div>
                            ...
                            <div className="mt-6 bg-gray-50 p-3 rounded-lg">
                                ...
                                <div className="mt-2 flex gap-2">
                                    <input value={pincode} onChange={(e) => setPincode(e.target.value)} placeholder="Enter PIN" className="px-3 py-2 border border-gray-300 rounded-md w-32 text-sm" />
                                    <button onClick={checkDelivery} className="px-3 py-2 bg-sky-900 text-white rounded-md text-sm cursor-pointer">Check</button>
                                </div>
                            ...
                            <div className="mt-6 flex items-center gap-3">
                                <div className="flex items-center border border-gray-300 rounded overflow-hidden">
                                    <button onClick={() => changeQuantity(-1)} className="px-3 py-2 cursor-pointer">-</button>
                                    <div className="px-6 py-2 w-14 text-center font-medium">{quantity}</div>
                                    <button onClick={() => changeQuantity(1)} className="px-3 py-2 cursor-pointer">+</button>
                                </div>

                                <button onClick={handleAddToCart} className="flex-1 px-4 py-3 rounded-lg bg-sky-900 text-white font-semibold flex items-center justify-center  cursor-pointer gap-2">
                                    <ShoppingCart className="w-5 h-5" /> Add to Cart
                                </button>
                            </div>

                            <button onClick={handleBuyNow} className="mt-3 w-full border border-gray-300 rounded-lg px-4 py-3 font-semibold cursor-pointer">Buy now</button>
                        ...
                        {!zoomOpen && <div className="lg:hidden fixed bottom-4 left-4 right-4 z-50">
                            <div className="bg-white p-3 rounded-xl shadow-lg flex gap-3">
                                ...
                                <button onClick={handleBuyNow} className="rounded-lg px-4 py-3 border border-r-gray-300">Buy</button>
                            </div>
                        </div>}
                ...
                <section className="mt-10 grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div className="lg:col-span-2">
                        <div className="bg-white border border-gray-300 rounded-lg p-6">
                            ...
                            <div className="mt-4 space-y-3 text-sm text-gray-700">
                                <details className="p-3 border rounded border-gray-300"><summary className="font-medium">Shipping time</summary><div className="mt-2 text-gray-600">Standard 3-7 days depending on location.</div></details>
                                <details className="p-3 border border-gray-300 rounded"><summary className="font-medium">Warranty</summary><div className="mt-2 text-gray-600">1 year on frame and mechanisms.</div></details>
                                <details className="p-3 border rounded border-gray-300"><summary className="font-medium">Returns</summary><div className="mt-2 text-gray-600">Free returns within 14 days.</div></details>
                            </div>
                        </div>

                        <div className="mt-6 bg-white border rounded-lg p-6 border-gray-300">
                            <h3 className="text-lg font-semibold">Customer reviews</h3>
                            <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                                {(product.reviewsList || [{ name: 'Ravi', rating: 5, text: 'Excellent build and comfort.' }]).map((r, i) => (
                                    <div key={i} className="border rounded border-gray-300 p-3">
                    ...
                    <aside className={"mb-20"}>
                        ...
                        <div className="mt-4 bg-white border rounded-lg border-gray-300 p-4 text-sm text-gray-700">
